...
from ..forms import EventVenueForm
...
def detail(request, record_id):
    ...
    Record details page
    '''
    template = 'pd_event/event_venue.html'
    record = get_object_or_404(Venue, pk=record_id)

    if request.method == 'POST':
        form = EventVenueForm(request.POST, instance=record)
        ...
        form = EventVenueForm(instance=record)
...
def add_new(request):
    ...
    template = 'pd_event/event_venue-add_new.html'
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.add_message(
                ...
                'list-group-item-success') 
            return redirect('pd_event:event_venue', record_id=record.id)
    ...
    return render(
        ...
        template, {
            ...
            'urls': {
                'add_new': 'pd_event:event_venue_add_new',
                'all_items': 'pd_event:event_venues'
            },
            'ajax': ajax,
            'base_template': base_template,
            'menu': draw_menu(cis_menu, 'events', 'pd_event:event_venue')
        })
...
def index(request):
    ...
    '''
    menu = draw_menu(cis_menu, 'events', 'pd_event:event_venue')
    template = 'pd_event/event_venue-list.html'
    ...
    return render(
        ...
        template, {
            ...
            'urls': {
                'add_new': 'pd_event:event_venue_add_new',
                'details_prefix': '/ce/cis/events/event_venue/'
            },
            'menu': menu,
            'api_url': '/ce/events/api/event_venues?format=datatables'
        }
    )
